...
from FoodApp.models import Product, Cart, OrderTable, CartTotal
...
def addcart(request):
    ...
    Cart.objects.get_or_create(pid=pid, name=name, quantity=quan, price=price,
                               total=Total, pic=image, user=usr)
    count = Cart.objects.filter(user=usr).aggregate(Count('name'))
    m_total = Cart.objects.all().aggregate(Sum('total'))
    CartTotal.objects.get_or_create(master_total=m_total['total__sum'])
...
def viewcart(request):
    ...
    c_t = CartTotal.objects.all()
    user = request.user
    ip = request.META['REMOTE_ADDR']
    if request.user.is_anonymous:
        user = ip
    cart = Cart.objects.filter(user=user)
    c_t = CartTotal.objects.filter(user=user)
    return render(request, 'FoodApp/cart.html', {'cart': cart, 'c_total': c_t})
...
def updatecart(request):
    ...
    Cart.objects.filter(pid=up_pid).update(
        quantity=up_quantity, price=up_price, total=up_total)
    cart = Cart.objects.all()
    user = request.user
    ip = request.META['REMOTE_ADDR']
    if request.user.is_anonymous:
...
def ordertab(request):
    ...
    for cart in c:
        ...
        OrderTable.objects.get_or_create(
            pid=id, name=name, quantity=quan, price=price, total=Total, user=user, pic=image)
    order = OrderTable.objects.all()
